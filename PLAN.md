...
## Redrawn Colombo and reconstructed corridor — 19 September 2026

The voyage map’s ship is now a plan-view steamship read from the 1901 Livorno dry-dock photograph and Rossini’s specification: plumb stem, single funnel abaft a low bridge house, foremast with a yard, pale hull with the exhibition’s red as the boot-topping line. It rotates with a smoothed heading, rolls gently under way, and releases funnel smoke that lifts from where it was released in map space. The route is written onto the map as the ship travels; each port mark and name pops in when the ship calls there, and the ship holds briefly at Lisbon, Cape Verde and Rio. The plate is larger and flat by default, with the tilt as the opt-in.

The corridor was re-traced. The Mediterranean and Portugal leg follows the printed 1903 steamship lanes after the earlier anchors proved miscalibrated there, and a coaling call at Cape Verde was added between Lisbon and the Atlantic leg. The Cape Verde call and the arrival dates offered in later research are reconstructions until the port-movement records they cite are located; the visible note says so. Reduced motion shows the completed route with all ports; nothing changes for readers without JavaScript beyond the new drawing.
